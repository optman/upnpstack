...
void RIODeviceDiscoverSink(struct UPnPDevice *device)
{
	...
	memset(riodevice,0,sizeof(struct RemoteIODevice));
	...
	*/
	...
	/* The following subscribes for events */
	RIOSubscribeForUPnPEvents(RIOGetService_RemoteIO(device),NULL);
	RIOSubscribeForUPnPEvents(RIOGetService_ChannelManager(device),NULL);
...
void RemoteIO_RegisterChannel(struct RemoteIODevice* device,char* URI,char* Name,int Time)
{
	RIOInvoke_ChannelManager_RegisterChannel(RIOGetService_ChannelManager(device->Device),&RIOResponseSink_ChannelManager_RegisterChannel,NULL,URI,Name,Time);
}

void RemoteIO_UnRegisterChannel(struct RemoteIODevice* device,char* URI)
{
	RIOInvoke_ChannelManager_UnregisterChannel(RIOGetService_ChannelManager(device->Device),&RIOResponseSink_ChannelManager_RegisterChannel,NULL,URI);
}
