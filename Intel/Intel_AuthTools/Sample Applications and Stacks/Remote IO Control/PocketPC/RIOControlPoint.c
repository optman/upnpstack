...
const char *UPNPCP_SOAP_Header = "POST %s HTTP/1.0\r\nHost: %s:%d\r\nUser-Agent: PPC2002, UPnP/1.0, Intel MicroStack/1.0.1186\r\nSOAPACTION: \"%s#%s\"\r\nContent-Type: text/xml\r\nContent-Length: %d\r\n\r\n";
...
struct packetheader *RIOBuildPacket(char* IP, int Port, char* Path, char* cmd)
{
	...
	ILibAddHeaderLine(RetVal,"User-Agent",10,"PPC2002, UPnP/1.0, Intel MicroStack/1.0.1186",44);
	FREE(HostLine);
	return(RetVal);
}
...
void RIOOnEventSink(void *ReaderObject, struct packetheader *header, char* buffer, int *BeginPointer, int BufferSize, int done, void* user)
{
	...
	if(done!=0)
	{
		...
		ILibAddHeaderLine(resp,"Server",6,"PPC2002, UPnP/1.0, Intel MicroStack/1.0.1186",44);
...
void RIORenew(void *state)
{
	...
	ILibAddHeaderLine(p,"User-Agent",10,"PPC2002, UPnP/1.0, Intel MicroStack/1.0.1186",44);
